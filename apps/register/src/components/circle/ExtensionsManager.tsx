...
// 2026-07-07: 旧「拡張機能 (モッド)」ダッシュボードページを廃止し、その管理UIを
// サークル設定内の1セクションへ統合した。モッドの導入・有効化・設定・削除は
// すべてここ (サークル設定 > 拡張設定) で行う。circle.mods (JSON) を読み書きする。
...
export function ExtensionsManager({ circleId }: { circleId: string }) {
  ...
  const updateModsMutation = useMutation({
    mutationFn: async (mods: ModsPayload) => circleApi.updateMods(circleId, mods),
    onSuccess: () => {
  ...
  const handleSave = () => updateModsMutation.mutate({ installed: installedMods });
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        const manifest = JSON.parse(event.target?.result as string) as ModManifest;
        installMod(manifest);
      } catch {
  ...
  const installMod = (manifest: ModManifest) => {
    ...
    setInstalledMods({
      ...installedMods,
      [manifest.id]: { manifest, enabled: false, settings: defaultSettings },
    });
  ...
  const updateSettingValue = (modId: string, key: string, value: any) => {
    setInstalledMods((prev) => {
      ...
      if (!mod) return prev;
      return { ...prev, [modId]: { ...mod, settings: { ...mod.settings, [key]: value } } };
    });
  };
  ...
  const toggleMod = (modId: string) => {
    setInstalledMods((prev) => {
      const mod = prev[modId];
      if (!mod) return prev;
      return { ...prev, [modId]: { ...mod, enabled: !mod.enabled } };
    });
  };
  ...
  return (
    <Card className="rounded-none shadow-none">
      <CardHeader className="pb-3 border-b-thick border-border">
        <CardTitle className="text-sm font-bold uppercase">拡張設定 (モッド)</CardTitle>
        <CardDescription className="text-xs text-muted-foreground">
          外部リポジトリなどから配布されたモッドを導入し、独自に拡張できます
        </CardDescription>
      </CardHeader>
      <CardContent className="pt-4 space-y-6 font-mono">
        {isLoading ? (
          ...
          <>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                <input type="file" id="mod-file-upload" accept=".json" onChange={handleFileUpload} className="hidden" />
                <Button
            ...
            <div className="space-y-4">
              ...
              {Object.keys(installedMods).length === 0 ? (
                <div className="border-thick border-dashed border-border p-8 text-center bg-muted">
                  ...
                  <p className="text-[10px] text-muted-foreground/80 mt-1">
                    上のボタンから、配布されているマニフェストのURLまたはJSONを入力して導入してください。
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                          ...
                          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                            ...
                            <div className="flex items-center gap-3 w-full sm:w-auto shrink-0">
                              <div className="flex items-center gap-2 border-thick border-accent-foreground/40 bg-background/10 px-3 h-9">
                                <span className="text-[10px] font-black uppercase w-8">{enabled ? "有効" : "無効"}</span>
                                <ToggleSwitch checked={enabled} onChange={() => toggleMod(modId)} label={`${manifest.name} の有効化`} />
                              </div>
                              <Button
                                type="button"
                                onClick={() => setPendingUninstall({ id: modId, name: manifest.name })}
              ...
              <div className="flex justify-end pt-4 border-t-thick border-border">
                ...
                >
                  ...
                  {updateModsMutation.isPending ? "保存中..." : "拡張設定を保存"}
                </Button>
              </div>
            </div>
          </>
